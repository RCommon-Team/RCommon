...
public class CachingGraphRepositoryTests
{
    ...
    private readonly CachingGraphRepository<TestEntity> _sut;

    public CachingGraphRepositoryTests()
    {
        ...
        _sut = new CachingGraphRepository<TestEntity>(_mockRepository.Object, _mockCacheFactory.Object);
    }
    ...
    public void Constructor_WithValidDependencies_CreatesInstance()
    {
        ...
        // Act
        var repository = new CachingGraphRepository<TestEntity>(mockRepository.Object, mockCacheFactory.Object);
    ...
    [InlineData(true)]
    [InlineData(false)]
    public void Tracking_Get_DelegatesToRepository(bool expectedValue)
    {
        // Arrange
        _mockRepository.Setup(x => x.Tracking).Returns(expectedValue);

        // Act
        var result = _sut.Tracking;
    ...
    [InlineData(true)]
    [InlineData(false)]
    public void Tracking_Set_DelegatesToRepository(bool value)
    {
        // Arrange & Act
        _sut.Tracking = value;

        // Assert
        _mockRepository.VerifySet(x => x.Tracking = value, Times.Once);
    }
    ...
    [Fact]
    public async Task AddRangeAsync_PassesCancellationToken()
    {
        // Arrange
        ...
        _mockRepository
            .Setup(x => x.AddRangeAsync(entities, It.IsAny<CancellationToken>()))
            .Callback<IEnumerable<TestEntity>, CancellationToken>((e, ct) => capturedToken = ct)
            .Returns(Task.CompletedTask);

        // Act
        await _sut.AddRangeAsync(entities, cts.Token);
    ...
    public async Task FindAsync_WithCacheKey_AndExpression_UsesCacheService()
    {
        ...
        // The cache service returns Task<TData> where TData is Task<ICollection<TestEntity>>
        _mockCacheService
    ...
    public async Task UpdateAsync_PassesCancellationToken()
    {
        ...
        // Act
        await _sut.UpdateAsync(entity, cts.Token);
    ...
    [Fact]
    public void CachingGraphRepository_ImplementsICachingGraphRepository()
    {
        // Act & Assert
        _sut.Should().BeAssignableTo<ICachingGraphRepository<TestEntity>>();
    }

    [Fact]
    public void CachingGraphRepository_ImplementsIGraphRepository()
    {
        // Act & Assert
        _sut.Should().BeAssignableTo<IGraphRepository<TestEntity>>();
    }
...
public class TestEntity : IBusinessEntity<Guid>
{
    ...
    public IReadOnlyCollection<ISerializableEvent> LocalEvents => new List<ISerializableEvent>();

    public void AddLocalEvent(ISerializableEvent eventItem) { }

    public void ClearLocalEvents() { }

    public bool EntityEquals(IBusinessEntity other) => other is TestEntity entity && entity.Id == Id;
