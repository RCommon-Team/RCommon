...
public class PublishWithMediatREventProducerTests
{
    ...
    [Fact]
    public async Task ProduceEventAsync_CallsMediatorServicePublish()
    {
        // Arrange
        var mockMediatorService = new Mock<IMediatorService>();
        mockMediatorService
    ...
    public async Task ProduceEventAsync_WithComplexEvent_PassesCorrectly()
    {
        ...
        var mockMediatorService = new Mock<IMediatorService>();
        mockMediatorService
            .Setup(x => x.Publish(It.IsAny<ComplexSerializableEvent>(), It.IsAny<CancellationToken>()))
