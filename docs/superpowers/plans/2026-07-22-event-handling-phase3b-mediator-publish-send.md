...
- (Path note) `EventRouteHandle` is at `Src/RCommon.Core/EventHandling/EventRouteHandle.cs` (namespace `RCommon.EventHandling`); `BuilderOutboxState` is inside `Routing/EventRoutingRegistry.cs`. All are internal to `RCommon.Core`, so the new public helper (also in Core) can use them directly — no `InternalsVisibleTo`, no visibility widening needed.
...
- `Publish<OrderConfirmed>().UseOutbox("Orders")` on the mediator builder ⇒ `IEventRoutingRegistry.TryGetOutboxStore(typeof(OrderConfirmed))` = "Orders" AND the event is targeted at `PublishWithMediatREventProducer`. **Assertion mechanism:** `EventSubscriptionManager.GetProducersForEvent(IEnumerable<IEventProducer>, Type)` FILTERS a supplied producer list — it does not resolve types from an event key alone. So assert either via `subscriptionManager.ShouldProduceEvent(typeof(PublishWithMediatREventProducer), typeof(OrderConfirmed))` (returns true), or by passing a fake producer list containing a `PublishWithMediatREventProducer` instance into `GetProducersForEvent` and asserting it survives the filter (matches the outbox router's own `FilterProducers` usage). Use `ShouldProduceEvent` for the simplest RED-compiling assertion.
