...
```
- Implementation: a `ConcurrentDictionary<Type, string>` (event type → store name). `MarkDurable` records/overwrites (later registration wins — matches "per-event overrides builder-level" when the builder default is applied first; see Task 3). Guard null/whitespace store name. `DurableStoreNames` = distinct values. Register as a **singleton** in `RCommonBuilder` next to `EventSubscriptionManager`.
...
**Design:**
- `UseRCommonOutbox(this IInMemoryEventBusBuilder builder, string dataStoreName)`: records a **builder-level default** durable store. Simplest mechanism: store the default on a small singleton keyed by builder type (e.g. reuse/extend a config-time holder) OR eagerly apply: any `Publish<T>()` **after** `UseRCommonOutbox` is called defaults to durable(store) unless the chained `.UseOutbox(...)` overrides it. **Precedence:** per-event `.UseOutbox(x)` overrides the builder default (last `MarkDurable` wins, and `Publish` applies the builder default *before* returning the handle so a subsequent `.UseOutbox` overwrites). Document the call-order requirement (builder-level `UseRCommonOutbox` should be declared before the `Publish<T>()` calls it governs — or make it order-independent by applying the default at build finalization; the simpler documented ordering is acceptable for 3.2.0).

- [ ] **Step 1: failing tests** — `UseRCommonOutbox("Orders")` then `Publish<T>()` ⇒ event durable to "Orders"; `UseRCommonOutbox("Orders")` then `Publish<T>().UseOutbox("Billing")` ⇒ event durable to "Billing" (per-event wins); `Publish<T>()` with no builder default and no `.UseOutbox` ⇒ transient.
- [ ] **Step 2:** run → fail.
- [ ] **Step 3:** implement.
- [ ] **Step 4:** run → pass.
...
**Design:** an `IHostedService` whose `StartAsync` compares `IEventRoutingRegistry.DurableStoreNames` against the registered outbox datastores (`IOutboxDataStoreRegistry.Registrations`, from Phase 1), case-insensitively. If a durable route names a store with no registered outbox, throw a descriptive exception (fail loud, consistent with the Phase-1 `OutboxSchemaVerificationHostedService` pattern — model on it). Register via `TryAddEnumerable`/idempotent add in `AddOutbox` (mirroring the Phase-1 diagnostic registration so multi-datastore `AddOutbox` yields one validator).
...
**Design:** Register `services.TryAddScoped<InMemoryTransactionalEventRouter>()` in `AddOutbox` (the concrete type, distinct from the `IEventRouter`→`OutboxEventRouter` registration). Inject it + `IEventRoutingRegistry` into `OutboxEntityEventTracker`. Keep all method bodies exactly as they are (Task 6 changes behavior). This isolates the risky wiring change (interface/ctor break + DI) from the behavior change, each independently green.

> **Guard:** `InMemoryTransactionalEventRouter` needs `IOptions<EventHandlingOptions>`, `IServiceProvider`, `ILogger<>`, `EventSubscriptionManager` — all already registered when core event handling is configured. Confirm the outbox host has core event handling (it always does — `AddOutbox` runs on `IPersistenceBuilder`, and event handling is configured via `WithEventHandling`). If resolution is uncertain, construct it via `ActivatorUtilities` inside the tracker instead of constructor injection — but prefer DI.
...
- `DispatchDomainEventsAsync` (pre-commit): subscribe each tracked graph `BusinessEntity`'s `TransactionalEventAdded` to a router-by-durability callback; seed each existing `LocalEvent` by the same rule; drain the **in-process** dispatcher (`InMemoryTransactionalEventRouter.RouteEventsAsync()`) so transient events are dispatched pre-commit, ordered, with the cycle-breaker. Unsubscribe in `finally`. Route-by-durability rule:
...
**Datastore-for-durable decision:** for an **entity-sourced** durable event, persist to the **entity's** datastore (`dataStoreName` from `TrackedEntitiesWithDataStore`) to preserve Phase-1 co-location/atomicity (AC-7); the `.UseOutbox("store")` name is validated at startup (Task 4) and expected to match. For a **router-added** durable event (no entity), the store from the routing registry is used. Document this; do not attempt cross-datastore reconciliation here.
...
- [ ] **Step 1: failing tests** (`RouteDrivenOutboxPipelineTests.cs`):
  ...
  - (d) An event with both a durable route and an `AddSubscriber` handler is persisted+relayed (subscriber fires during relay), not double-delivered.
