...
- Add a small config-time state holder tracking, keyed **per builder type** (`builder.GetType()`, consistent with how `EventSubscriptionManager` keys subscriptions): the builder-level default store (nullable) and the set of event types `Publish`'d on that builder that were **explicitly** given a per-event `.UseOutbox` (so the default never clobbers an explicit choice). Store it on the routing registry (add internal helpers) or a companion config-time singleton looked up like `GetRoutingRegistry`. (Per-type keying matches existing conventions; multiple in-memory bus builders of the same type in one container is not a supported scenario.)
