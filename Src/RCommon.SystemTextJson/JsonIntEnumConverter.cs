...
namespace RCommon.SystemTextJson
{
    ...
    /// <see cref="int"/> numeric representation and deserializes from string names.
    /// </summary>
    /// <typeparam name="T">The enum type to convert.</typeparam>
    /// <remarks>
    /// During reading, the converter expects the JSON token to be a string containing the enum member name.
    /// During writing, the converter outputs the numeric <see cref="int"/> value of the enum member.
    /// </remarks>
    /// <seealso cref="JsonByteEnumConverter{T}"/>
    public class JsonIntEnumConverter<T> : JsonConverter<T> where T : Enum
    {
        ...
        /// <summary>
        /// Writes the enum value as its <see cref="int"/> numeric representation.
        /// </summary>
        ...
        {
            // Re-parse the enum to its base Enum type and convert to int for numeric output
