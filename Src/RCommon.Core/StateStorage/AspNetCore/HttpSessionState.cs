...
namespace RCommon.StateStorage.AspNetCore
{
    ...
    {
        //readonly ISystemContext _environmentAccessor;
        ...
        /// Default Constructor.
        /// </summary>
        public HttpSessionState(IHttpContextAccessor httpAccessor)
        {
        ...
        public T Get<T>(object key)
        {
            ...
            /*_logger.Debug(x => x("Attempting to get {0} from session state for session {1}", 
                fullKey, _environmentAccessor.HttpContext.Session.SessionID));*/
            //lock (_session.K.HttpContext.Session.SyncRoot)

            return (T)JsonSerializer.Deserialize<T>(_session.GetString(fullKey));
        }
        ...
        public void Put<T>(object key, T instance)
        {
            ...
            /*_logger.Debug(x => x("Attempting to put {0} to session state for session {1}",
                fullKey, _environmentAccessor.HttpContext.Session.SessionID));*/
            /*lock (_environmentAccessor.HttpContext.Session.SyncRoot)
                _environmentAccessor.HttpContext.Session[fullKey] = instance;*/
            _session.SetString(fullKey, JsonSerializer.Serialize<T>(instance));
        }
        ...
        public void Clear()
        {
            //_logger.Debug(x => x("Attempting to clear session state for session {1}",_environmentAccessor.HttpContext.Session.SessionID));
            _session.Clear();
            //_environmentAccessor.HttpContext.Session.Clear();
        }
    }
}