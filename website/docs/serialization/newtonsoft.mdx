...
sidebar_position: 2
description: RCommon.JsonNet wraps Newtonsoft.Json behind IJsonSerializer with full JsonSerializerSettings access, custom converters, and camelCase or indented output options.
---
...
`WithJsonSerialization<JsonNetBuilder>()` is a singleton-style verb across all six overloads (parameterless, `serializeOptions`, `serializeOptions + deSerializeOptions`, `actions`, `serializeOptions + actions`, and `serializeOptions + deSerializeOptions + actions`). Only one `IJsonSerializer` implementation makes sense per process, so the verb enforces the choice at startup.

| Re-registration shape | Behaviour |
|---|---|
| `WithJsonSerialization<JsonNetBuilder>()` called from two modules | Idempotent no-op — the second registration is skipped, but the configuration delegate still runs so per-call option values follow last-write-wins. |
| One module calls `WithJsonSerialization<JsonNetBuilder>()` and another calls `WithJsonSerialization<TextJsonBuilder>()` | Throws `RCommonBuilderException` naming both builder types. Pick exactly one JSON serializer. |
