...
sidebar_position: 3
description: Implement event-driven .NET apps with RCommon using IEventProducer and ISubscriber across in-memory, MediatR, MassTransit, and Wolverine providers.
---
...
services.AddRCommon()
    ...
    {
        eventHandling.Publish<LeaveRequestApprovedEvent>();
        eventHandling.AddSubscriber<LeaveRequestApprovedEvent, SendApprovalEmailHandler>();
    });
```
...
services.AddRCommon()
    .WithEventHandling<MassTransitEventHandlingBuilder>(eventHandling =>
    {
        ...
        eventHandling.Consume<LeaveRequestApprovedEvent, SendApprovalEmailHandler>();
    });
```
...
services.AddRCommon()
    ...
    .WithEventHandling<MassTransitEventHandlingBuilder>(masstransit =>
    {
        ...
        masstransit.Consume<PayrollSyncRequiredEvent, SyncPayrollHandler>();
    });
```
...
services.AddRCommon()
    ...
    .WithEventHandling<MassTransitEventHandlingBuilder>(eventHandling =>
    {
        ...
        eventHandling.Consume<MassTransitOnlyEvent, MassTransitOnlyEventHandler>();
        eventHandling.Consume<SharedEvent, SharedEventHandler>();
    });
```
...
For full outbox durability (where events survive process crashes between the write and the publish), durability is **opt-in per route** in RCommon 3.2.0: a plain `Publish`/`Send` route is transient, while chaining `.UseOutbox("<datastore>")` onto a route (or setting a builder-level default with `UseRCommonOutbox("<datastore>")` / `UseBrokerOutbox(...)`) persists the event before it leaves. See the [event-handling recipes](../event-handling/recipes.mdx) for the RCommon and broker-native outbox variants.
