...
The [End-to-End DDD Recipe](../examples-recipes/domain-driven-design.mdx) uses an `EmailAddress : ValueObject<string>` on `TeamMembership`, with a validating factory method and tests proving structural equality.
:::
