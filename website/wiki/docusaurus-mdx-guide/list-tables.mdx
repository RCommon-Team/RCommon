...
If the table consists of simple text and no complex UI and no column width control. This can be plain markdown syntax as
below:
...
When a table cell is just a codeblock, the normal triple tick syntax works fine. But, please do leave an intentional
empty line before and after the code block. Also, start the code block and respective cell (`td/th`) at the very
beginning of line (no empty tabs/spaces at start).
...
Do one better and ignore the indentation altogether for all tags in table. This might be bit of not so eye-friendly to
read, but works. Please do not that this is required only when there is a cell involving code block.
