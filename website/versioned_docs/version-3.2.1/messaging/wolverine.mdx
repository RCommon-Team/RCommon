...
title: Wolverine
sidebar_position: 5
description: Configure Wolverine as RCommon's messaging transport with durable delivery, fan-out publish or point-to-point send, and ISubscriber handlers free of Wolverine dependencies.
---
...
`Consume<TEvent, TEventHandler>` registers `ISubscriber<TEvent>` as scoped in the DI container and records the event-to-consumer association in the `EventSubscriptionManager`. `AddSubscriber<TEvent, TEventHandler>` is retained as an `[Obsolete]` alias that forwards to `Consume` for pre-3.2.0 source compatibility.
...
Use `Send<TEvent>()` for command-style messaging; it auto-registers `SendWithWolverineEventProducer`:
...
| `Consume<TEvent, THandler>` | `RCommon.Wolverine` | Extension on `IWolverineEventHandlingBuilder`; registers the inbound consumer, handler, and routing (`AddSubscriber` is the `[Obsolete]` alias) |
