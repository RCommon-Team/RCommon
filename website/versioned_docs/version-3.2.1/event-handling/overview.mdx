...
:::tip Looking for a pattern to follow?
The [Event Handling Recipes](./recipes.mdx) guide walks through the six supported patterns (DDD with the RCommon outbox, broker-as-producer, broker-native outbox, transaction-script, no-unit-of-work, and in-process MediatR) with wiring and diagrams. If you are upgrading from an earlier version, start with the [3.2.0 migration section](../api-reference/migration-guide.mdx#upgrading-to-320-event-handling--outbox-redesign) — durability is now opt-in.
:::
