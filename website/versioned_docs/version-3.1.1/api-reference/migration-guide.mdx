...
Both modules call `AddRCommon()` and `WithPersistence<EFCorePerisistenceBuilder>`. The persistence sub-builder's constructor runs exactly once; both `AddDbContext` calls accumulate registrations.
