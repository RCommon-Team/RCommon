...
After calling `WithPersistence<EFCorePersistenceBuilder>`, the open-generic repository interfaces are available for injection anywhere:
...
Both `WithPersistence<EFCorePersistenceBuilder>` calls share the same persistence sub-builder, so both `AddDbContext` calls accumulate against one `DataStoreFactoryOptions`. The duplicate `WithSimpleGuidGenerator()` is a no-op because the impl type matches.

See [Modular Composition](../core-concepts/modular-composition.mdx) for the full conflict matrix and `Examples/Bootstrapping/Examples.Bootstrapping.MultiModule/` for a runnable three-module example.
