...
:::note `ISyncEvent`/`IAsyncEvent` control dispatch ordering, not transport
...
| `ISyncEvent` | Marker; selects **sequential** dispatch ordering (not a transport choice — still fully serializable / outbox-eligible) |
| `IAsyncEvent` | Marker; selects **concurrent** dispatch ordering (not a transport choice) |
